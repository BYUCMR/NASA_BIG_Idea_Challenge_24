...
[ x_temp, Edges_Tube, B_T, n_true, Edges_True] = Generate_Shape_Order_Simple( Nodes, L_tube );
...
figure
Plot_Edges(Edges_Tube, x_all,'o-')  %All of the Required Edges
hold on
Number_Nodes(x_all)
axis equal
title('Nodes')
...
C_Lock(1,Support(1))=1;
C_Lock(2,Support(1)+n_all)=1;
C_Lock(3,Support(1)+2*n_all)=1;

C_Lock(4,Support(2)+n_all)=1;
C_Lock(5,Support(2)+2*n_all)=1;

C_Lock(6,Support(3)+2*n_all)=1;
...
for Movement_i=1:N_Moves
    ...
    Loop_Con=[];
    N_subsections = N_Edges_True/3;
    for i=1:N_subsections
        Elements=3;
        Loop_Con=blkdiag(Loop_Con, ones(1,Elements));
    end

        for it_t=1:length(0:dt:t_move{Movement_i}) %to test 2 steps
...
%             Turn_On_Broadcast=data.broadcast_command.Value;
        ...
        Cent_Decentralized_On=1;
            ...
            [ x_opt, Ldot, err ] = Controled_Motion_Nodes_Constraints( y_tot(end,:)', Edges_All, Loop_Con, L2th, A, b, L_norm, Cost_Fun_Index, Adj);
        else
        ...
        if err == 1
            disp("exiting movement loop");
            disp(it_t + "/" + length(0:dt:t_move{Movement_i}));
            break;
        end
...
L = Get_Lengths_E(Edges_All, reshape(y_tot(end,:)',n_all,3))
...
% node_plot=2;
clf
Plot_Edges(Edges_All, reshape(y_tot(1,:)',n_all,3),'-r')
axis equal
hold on
