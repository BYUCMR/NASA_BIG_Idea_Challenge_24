...
A_com{1}=M_Mat(1:2,:);
b_com{1}=[0 -1]'; %Where to move the center of mass
%Also use the ground constraint matrix
C_Lock=zeros(9,3*n_all);
%Constrained Motion of a few nodes
% A=[A_Node{1}; A_com{1}; C_Lock];
% b=[b_node{1}; b_com{1} ; b_Lock];
Ind_Support=[1 2 5]; %The Support Polygon Base
for i=1:length(Ind_Support)
        C_Lock(3*(i-1)+1,Ind_Support(i))=1;
        C_Lock(3*(i-1)+2,Ind_Support(i)+n_all)=1;
        C_Lock(3*(i-1)+3,Ind_Support(i)+2*n_all)=1;
end
A=[ A_com{1}; C_Lock];
b=[ b_com{1}; zeros(9,1)];
...
Loop_Con=[];
for i=1:Num_Circuits
    Elements=size(Edge_Tube{i},1);
    Loop_Con=blkdiag(Loop_Con, ones(1,Elements));
end

%% Determine practice motions for how things should move
t=1;
L2th=[];
Order=T2T_Qual;
y_tot=reshape(x_all,3*n_all,1)';
Ans=Controled_Motion( t, y_tot, Edges_All, Edge_All_Tube, Loop_Con, L2th, Order, A, b);
%The problem is non-convex with just th eH matrix as the objective.
%Is that expected or no? 
%%
...
%     Dynamics=@(t,x)Dynamics_Double_Roller( t, x, Edges_All, C, T2T_Qual, input_total );
    ...
    Dynamics=@(t,x)Controled_Motion( t, y_tot, Edges_All, Edge_All_Tube, Loop_Con, L2th, Order, A, b);
