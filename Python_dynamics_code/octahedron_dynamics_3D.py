...
class OctahedronDynamics:
    ...
    def f(self, state, tau = np.zeros((18,1))):
        ...
        # Construct the force vector and add the external forces (18x1 column vector of forces applied in the x/y/z directions to nodes 1, 2, and/or 3)
        F = Fs + Fb
        ...
        y1ddot = (1/P.m)*(sum_of_forces[1])
        z1ddot = (1/P.m)*(sum_of_forces[2])
        x2ddot = (1/P.m)*(sum_of_forces[3])
        y2ddot = (1/P.m)*(sum_of_forces[4])
        z2ddot = (1/P.m)*(sum_of_forces[5])
        x3ddot = (1/P.m)*(sum_of_forces[6])
        y3ddot = (1/P.m)*(sum_of_forces[7])
        z3ddot = (1/P.m)*(sum_of_forces[8])
        x4ddot = (1/P.m)*(sum_of_forces[9])
        y4ddot = (1/P.m)*(sum_of_forces[10])
        z4ddot = (1/P.m)*(sum_of_forces[11])
        x5ddot = (1/P.m)*(sum_of_forces[12])
        y5ddot = (1/P.m)*(sum_of_forces[13])
        z5ddot = (1/P.m)*(sum_of_forces[14])
        x6ddot = (1/P.m)*(sum_of_forces[15])
        y6ddot = (1/P.m)*(sum_of_forces[16])
        ...
        xdot = np.array([[x1_dot], [y1_dot], [z1_dot], [x2_dot], [y2_dot], [z2_dot], [x3_dot], [y3_dot], [z3_dot], [x4_dot], [y4_dot], [z4_dot], [x5_dot], [y5_dot], [z5_dot], [x6_dot], [y6_dot], [z6_dot],
                         [x1ddot], [y1ddot], [z1ddot], [x2ddot[0]], [y2ddot], [z2ddot], [x3ddot[0]], [y3ddot[0]], [z3ddot], [x4ddot[0]], [y4ddot[0]], [z4ddot[0]], [x5ddot[0]], [y5ddot[0]], [z5ddot[0]], [x6ddot[0]], [y6ddot[0]], [z6ddot[0]]])
        return xdot
