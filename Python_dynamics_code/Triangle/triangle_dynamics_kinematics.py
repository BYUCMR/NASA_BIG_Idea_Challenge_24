...
with open('Users/annieobryan/Desktop/NASA Big Idea Challenge/Integrated Kinematics and Dymamics/triangle_positions.csv') as f:
    output = [float(s) for line in f.readlines() for s in line[:-1].split(',')]

kinematic_positions = np.array(output).reshape(1002,6)
...
class TriangleDynamics:
    ...
    def __init__(self):
        ...
        self.num_nodes = int(np.size(self.RM.x)/2)
        initial_node_positions = kinematic_positions[0, :]
        x_i = np.zeros((self.num_nodes, 2))
        for i in range(self.num_nodes):
            x_i[i] = np.array([initial_node_positions[i]])
    ...
    def f(self, state, tau=np.zeros((6))):
        ...
        x_k = np.zeros((self.num_nodes, 2))
        for i in range(self.num_nodes):
            x_k[i] = np.array([x_kinematics[i], x_kinematics[i + self.num_nodes]])
