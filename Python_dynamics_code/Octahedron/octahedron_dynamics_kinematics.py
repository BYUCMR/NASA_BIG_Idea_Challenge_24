...
with open('/Users/annieobryan/Desktop/NASA Big Idea Challenge/Integrated Kinematics and Dynamics/octahedron.csv') as f:
    output = [float(s) for line in f.readlines() for s in line[:-1].split(',')]

kinematic_positions = np.array(output).reshape(127,18)
...
class OctahedronDynamics:
    ...
    def f(self, state, tau = np.zeros((18))):
        ...
        if self.i < 127:
            ...
            x_kinematics = kinematic_positions[126, :]
