...
typedef enum {DC_Automatic, DC_Manual} DCMotorControlState_t;        // different states the controller can be put in.
...
public:
    ...
    void    setControlRate(int NewControlRate);		// set the member variable that stores the control rate. This should match the rate that run is called.
    ...
    void    setDeadbandTicks(int DeadbandTicks);			// If the current position is within the deadband of the desired position, the motors will not move.
		void 		setDeadbandTicks(int InnerDeadbandTicks, int OuterDeadbandTicks); // two deadband layers. the motors stop when inside the inner layer and dont start gain until outside the outer layer.
    ...
    int     getDesiredPositionTicks();
    float   getDesiredPositionInches();
    int     getCurrentPositionTicks();
    ...
    int     getControlRate();
    int     getDeadbandTicks();
    int     getDeadbandDutyCycle();
